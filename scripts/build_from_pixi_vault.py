...
def search_script() -> str:
    ...
    case-insensitive AND-token whole-word matching. Exact and containing title
    phrases rank above path and namespace matches. The dropdown renders at most
...
def search_page_script() -> str:
    ...
    same registry the header dropdown uses (``data-registry``). Matching uses the
    same case-insensitive AND-token whole-word rule; results are RANKED with
    exact-title and title-phrase boosts, then path and namespace evidence, then
    title ASC, with matched tokens wrapped in ``<mark>`` via DOM text nodes
